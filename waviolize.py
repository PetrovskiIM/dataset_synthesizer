...
from config import train_wav_storage_path, test_wav_storage_path, \
    different_instrument_csv_path as template_csv_path, \
    instrument_files_names as template_names
import pandas as pd
...
if __name__ == '__main__':
    ...
    in_train_names = unique_names[:1000]
    in_test_names = unique_names[1000:1010]
    train_in_out_names = unique_names[1000:]
    out_names = unique_names[1010:]
    ...
    with Pool(5) as p:
        print(p.map(waviolize_train, paths))
    ...
    with Pool(5) as p:
        print(p.map(waviolize_test, paths))
